...
contract ZwangLocker is Ownable, ReentrancyGuard {
    ...
    uint256 public OEthFee = 1 ether;
    // base 1000, 0.5% = value * 5 / 1000
    uint256 public lpFeePercent = 5;

    // for statistic
    uint256 public totalOEthFees = 0;
    // withdrawable values
    ...
    function lockTokens(
        ...
        require(!_feeInOEth || msg.value > OEthFee, 'OEth fee not provided');
        ...
        if (_feeInOEth) {
            totalOEthFees = totalOEthFees.add(msg.value);
            remainingOEthFees = remainingOEthFees.add(msg.value);
        } else {
    ...
    function setOEthFee(uint256 fee) external onlyOwner {
    ...
    function withdrawFees(address payable withdrawalAddress) external onlyOwner {
        if (remainingOEthFees > 0) {
            withdrawalAddress.transfer(remainingOEthFees);
            remainingOEthFees = 0;
        }
